...
export default {
	async fetch(request, env, ctx) {
		...
		if (env.GO2DIRECT) SOCKS5直连白名单 = await 整理成数组(env.GO2DIRECT);
		else SOCKS5直连白名单 = [];
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (访问路径 === 'admin/log.json') {// 访问日志已从源码关闭
					...
					if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							return new Response('订阅转换功能已从源码关闭，请使用 mixed 原始订阅。', { status: 403 });
						}
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID) {
	...
	const 命中分流规则 = (addr, 规则数组 = []) => 规则数组.some(p => {
		const 规则 = String(p ?? '').trim();
		if (!规则) return false;
		const 正则规则 = 规则.replace(/[|\\{}()[\]^$+?.]/g, '\\$&').replace(/\*/g, '.*');
		return new RegExp(`^${正则规则}$`, 'i').test(addr);
	});
	const 命中直连白名单 = 命中分流规则(host, SOCKS5直连白名单);
	const 需要走代理 = 启用SOCKS5反代 ? !命中直连白名单 : false;

	if (需要走代理) {
		log(`[TCP转发] 命中代理策略，走 SOCKS5/HTTP/HTTPS 代理`);
		try {
			await connecttoPry();
		} catch (err) {
...
async function 请求日志记录(env, request, 访问IP, 请求类型 = "Get_SUB", config_JSON, 是否写入KV日志 = true) {
	return; // 源码硬关闭访问日志与 Telegram 通知，避免敏感路径、token 或代理账号外发/落盘。
}
